...
public class EICBookParsingProfile extends SiteParsingProfile implements SpecificProfile {
	...
	public Title scrapeTitle() {
		...
		if (doGoogleTranslation) {
			return new Title(TranslateString
					.translateStringJapaneseToEnglish(titleElement.text().replaceAll("(" + whitespace_charclass + "Blu-ray版|" + whitespace_charclass + "DVD版)", "")));
		} else {
	...
	public ReleaseDate scrapeReleaseDate() {
		Element releaseDateElement = document.select("tr:has(th:contains(発売日)) td a").first();
	...
	public moviescraper.doctord.model.dataitem.Runtime scrapeRuntime() {
		String runtime = "";
		Element runtimeElement = document.select("tr:has(th:contains(収録時間)) td a").first();
	...
	private Thumb[] scrapePostersAndFanart(boolean doCrop, boolean scrapingExtraFanart) {
		...
		Element postersElement = document.select("#dtTopL a").first();
		if (postersElement == null) {
			postersElement = document.select("#dtTopL img").first();
		}
	...
	public ArrayList<Genre> scrapeGenres() {
		Elements genreElements = document.select("tr:has(th:contains(ジャンル)) td a");
		...
		for (Element genreElement : genreElements) {
			...
			String genreID = genreElement.attr("href").substring(href.indexOf("genre_name=") + 11, href.length());
	...
	public ArrayList<Actor> scrapeActors() {
		...
		Elements actressIDElements = document.select("tr:has(th:contains(出演者)) td a");
	...
	public ArrayList<Director> scrapeDirectors() {
		...
		Element directorElement = document.select("tr:has(th:contains(監督・スタッフ)) td a").first();
	...
	public Studio scrapeStudio() {
		Element studioElement = document.select("tr:has(th:contains(メーカー)) td a").first();
	...
	public String createSearchString(File file) {
		...
		try {
			...
			return "http://www.eic-book.com/search?q=" + fileNameURLEncoded;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public SearchResult[] getSearchResults(String searchString) throws IOException {
		...
		while (firstPageScraping || nextPageLink != null) {
			...
			Elements dvdLinks = searchResultsPage.select("div.cmnList div.list a[href*=/product/detail/]:has(.PH)");
