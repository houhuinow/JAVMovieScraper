...
public class DmmComParsingProfile extends SiteParsingProfile implements SpecificProfile {
		...
		try {
			...
			if (scrapeTrailers && (buttonElement = document.select("a.d-btn[onclick*=sampleplay]").first()) != null) {
				...
				if (matcher.matches()) {
					...
					String potentialTrailerURL = String.format("http://cc3001.dmm.com/litevideo/freepv/%1$s/%2$s/%3$s/%3$s_%4$s_%5$s.mp4", firstLetterOfCid, threeLetterCidCode,
							cid, quality, ratio);
	...
	private Thumb[] scrapePostersAndFanart(boolean doCrop, boolean scrapingExtraFanart) {
		...
		Element postersElement = document.select("a[href^=http://pics.dmm.com][name=package-image], div#sample-video img[src*=/pics.dmm.com]").first();
		...
		ArrayList<Thumb> posters = new ArrayList<Thumb>(1 + extraArtElementsSmallSize.size());
	...
	public ArrayList<Genre> scrapeGenres() {
		...
		for (Element genreElement : genreElements) {
			...
			String genreID = genreElement.attr("href").substring(href.indexOf("id=") + 3, href.length() - 1);
	...
	public ArrayList<Actor> scrapeActors() {
		...
		Elements actressIDElements = document.select("a[href*=article=actor/id=]");
		ArrayList<Actor> actorList = new ArrayList<Actor>(actressIDElements.size());
	...
	public Studio scrapeStudio() {
		Element studioElement = document.select("table.mg-b20 tr td a[href*=article=maker/id=]").first();
	...
	public String createSearchString(File file) {
		...
		try {
			...
			return "http://www.dmm.com/search/=/searchstr=" + fileNameURLEncoded + "/";
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
		...
		while (firstPageScraping || nextPageLink != null) {
			...
			for (int i = 0; i < dvdLinks.size(); i++) {
				...
				if (imageLinkElement != null) {
					String url = imageLinkElement.attr("src");
					if (url.startsWith("/")) {
						url = "http://www.dmm.com" + url;
					}
					Thumb currentPosterThumbnail = new Thumb(url);
